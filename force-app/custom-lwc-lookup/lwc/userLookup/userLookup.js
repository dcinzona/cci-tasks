...
const DEFAULT_ICON = 'standard:user';
...
export default class UserLookup extends LightningElement {
    ...
    @api lookupFieldApiName; // Field API Name (Primary and Required)
    ...
    */
    ...
    connectedCallback() {
        ...
        this.isLoading = true;
        if (this.recordId && this.lookupFieldApiName) {
            getCurrentValueForRecord({ recordId: this.recordId, userLookupField: this.lookupFieldApiName, subField: this.subFieldApiName })
                .then(result => {
                    console.log('result', result);
                    this.value = [{ id: result.Id, title: result.title, subtitle: result.subtitle }];
